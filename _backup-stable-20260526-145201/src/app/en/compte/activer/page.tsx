...
function ActivateContent() {
  ...
  async function activate() {
    try {
      ...
      const { error } = await supabase.auth.signUp({
        ...
        password,
        options: {
          emailRedirectTo: `${window.location.origin}/en/auth/callback?next=/en/espace-client`,
          data: {
  ...
  return (
    <section className="py-16">
      <div className="mx-auto max-w-4xl px-6">
        <div className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/70">
          <p className="text-sm font-black uppercase tracking-[0.18em] text-[#123A63]">
          ...
          <p className="mt-4 text-sm font-semibold leading-7 text-slate-500">
            ...
            vous pourrez vous connecter Ã  votre espace client.
          </p>

          {account && (
            <div className="mt-7 rounded-2xl bg-[#fff7f1] p-5">
              <p className="text-xs font-black uppercase tracking-[0.16em] text-[#123A63]">
                Compte Ã  activer
              </p>
          ...
          <div className="mt-7 grid gap-5">
            <label>
              ...
              <input
                type="password"
                className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-4 text-sm font-bold outline-none focus:border-[#123A63] focus:ring-4 focus:ring-orange-100"
                value={password}
            ...
            <label>
              ...
              <input
                ...
                placeholder="Répéter le Password"
              />
            </label>
            ...
            <button
              ...
              className="rounded-2xl bg-[#F15A24] px-6 py-4 text-sm font-black text-white shadow-lg shadow-orange-900/10 disabled:cursor-not-allowed disabled:bg-slate-300"
            >
              {status === "sending"
                ? "Envoi de l'email..."
                : status === "sent"
                  ? "Email sent"
                  : "Create my account and receive the email"}
            </button>
          </div>
          ...
          {status === "sent" && (
            <div className="mt-6 flex gap-3">
              <a
                href="/en/connexion"
                className="rounded-2xl border border-slate-200 bg-white px-6 py-4 text-sm font-black text-slate-900 hover:bg-[#fff7f1]"
              >
                Go to sign in
              </a>
            </div>
