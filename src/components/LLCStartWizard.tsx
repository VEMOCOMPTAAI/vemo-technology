  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="flex w-full items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-4 text-left text-sm font-black outline-none transition hover:border-[#F15A24]"
      >
        ...
        <div className="absolute z-50 mt-2 w-full overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-300/60">
          <div className="border-b border-slate-100 p-3">
            <input
              ...
              className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm font-bold outline-none focus:border-[#F15A24] focus:ring-4 focus:ring-[#F15A24]/10"
            />
          </div>
          ...
          <div className="max-h-[260px] overflow-y-auto p-2">
            {filtered.map((country) => (
              <button
                ...
                className={cn(
                  "flex w-full items-center justify-between gap-3 rounded-xl px-3 py-3 text-left text-sm font-black hover:bg-white",
                  selected.code === country.code && "bg-white text-[#F15A24]"
                )}
              >
                <span className="truncate">
      ...
      <div className="grid gap-3 md:grid-cols-[210px_1fr]">
        <div className="relative">
          <button
            type="button"
            onClick={() => setOpen((current) => !current)}
            className="flex w-full items-center justify-between gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-4 text-left text-sm font-black outline-none transition hover:border-[#F15A24] focus:border-[#F15A24] focus:ring-4 focus:ring-[#F15A24]/10"
          >
            <span className="truncate">
              ...
              <div className="max-h-[260px] overflow-y-auto p-2">
                {filtered.map((item) => (
                  <button
                    ...
                    className={cn(
                      "flex w-full items-center justify-between gap-3 rounded-xl px-3 py-3 text-left text-sm font-black hover:bg-white",
                      selected.code === item.code && "bg-white text-[#F15A24]"
                    )}
                  >
                    <span className="truncate">
                      {item.flag} {item.name}
    ...
    <label className="block">
      ...
      <div className="relative">
        <select
          value={value}
          onChange={(event) => onChange(event.target.value)}
          className="w-full appearance-none rounded-2xl border border-slate-200 bg-white px-5 py-4 pr-10 text-sm font-black outline-none transition focus:border-[#F15A24] focus:ring-4 focus:ring-[#F15A24]/10"
        >
          {options.map((option) => (
...
const canContinue = useMemo(() => {
  ...
  const resolvedBillingName = form.billingName || form.fullName || form.memberName || "";
  const resolvedBillingEmail = form.billingEmail || form.email || "";
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    active ? "border-[#F15A24] bg-white" : done ? "border-[#E8E2DC] bg-white" : "border-slate-200 bg-white"
                  )}
                >
                  <span className={cn("flex h-9 w-9 items-center justify-center rounded-full text-xs font-black", active ? "bg-[#F15A24] text-white" : done ? "bg-white text-[#F15A24]" : "bg-slate-100 text-slate-400")}>
        ...
        <div className="grid gap-7 lg:grid-cols-[1fr_420px]">
          <section className="rounded-[2rem] border border-slate-100 bg-white p-7 shadow-xl shadow-slate-200/70 md:p-10">
                ...
                <div className="mt-8 grid gap-5 md:grid-cols-3">
                  {plans.map((plan) => (
                    <button
                      ...
                      className={cn("relative rounded-[1.5rem] border p-6 text-left transition", form.plan === plan.id ? "border-[#F15A24] bg-white" : "border-slate-200 bg-white")}
                    >
                      {plan.featured && (
                        <span className="rounded-full bg-[#F15A24] px-3 py-1 text-xs font-black uppercase tracking-wide text-white">
                          {t.popular}
                        </span>
                      )}
                ...
                <div className="mt-8 grid gap-5 md:grid-cols-2">
                  {(["New Mexico", "Wyoming"] as Jurisdiction[]).map((state) => (
                    <button
                      ...
                      className={cn("rounded-[1.5rem] border p-7 text-left transition", form.jurisdiction === state ? "border-[#F15A24] bg-white" : "border-slate-200 bg-white")}
                    >
              ...
              <div>
                ...
                <label className="mt-6 block">
                  ...
                  <textarea
                    ...
                    className="min-h-[150px] w-full rounded-2xl border border-slate-200 bg-white px-5 py-4 text-sm font-black outline-none transition focus:border-[#F15A24] focus:ring-4 focus:ring-[#F15A24]/10"
                  />
                </label>
              </div>
            )}
              ...
              <div>
                ...
                <div className="mt-8 grid gap-5 md:grid-cols-2">
                  {availableServices.map((service) => (
                    <div key={service.id} className="rounded-[1.5rem] border border-[#F15A24] bg-white p-6 shadow-lg shadow-slate-200/70">
            ...
            {step === 10 && (
              <div>
                ...
                <div className="mt-8 grid gap-5 md:grid-cols-2">
                  <Field label={t.billingName} value={resolvedBillingName} onChange={(value) => update("billingName", value)} placeholder={t.billingName} />
                  <Field label={t.billingEmail} value={resolvedBillingEmail} onChange={(value) => update("billingEmail", value)} placeholder="billing@domain.com" type="email" />
                  <SelectField
            ...
            <div className="mt-9 flex justify-between border-t border-slate-100 pt-6">
              ...
              {step < 10 && (
                <button
                  type="button"
                  onClick={next}
                  className={cn("rounded-2xl px-6 py-3 text-sm font-black text-white shadow-lg", canContinue ? "bg-[#F15A24] shadow-orange-900/20" : "cursor-not-allowed bg-slate-300 shadow-none")}
                >
                  {t.continue}
                </button>
              )}
          ...
          <aside className="h-fit rounded-[2rem] border border-slate-100 bg-white p-7 shadow-xl shadow-slate-200/70">
            ...
            <div className="mt-6 h-3 rounded-full bg-slate-100">
              <div className="h-3 rounded-full bg-[#F15A24]" style={{ width: `${progress}%` }} />
            </div>
            <p className="mt-3 text-xs font-black uppercase tracking-[0.2em] text-[#F15A24]">
              {t.progress} : {progress}%
            </p>
            ...
            <div className="mt-7 rounded-[1.5rem] border border-[#E8E2DC] bg-white p-5">
              <div className="flex items-center justify-between gap-4">
                <p className="text-xl font-black">{t.estimatedTotal}</p>
                <p className="text-5xl font-black tracking-[-0.08em] text-[#F15A24]">
                  {money(total)}
                </p>
              </div>
            </div>
