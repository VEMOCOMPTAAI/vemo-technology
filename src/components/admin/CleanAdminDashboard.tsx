...
function normalizeArray(data: any): AdminItem[] {
  const possible = data?.orders || data?.clients || data?.dossiers || data?.items || data?.data || data?.rows || [];

  if (!Array.isArray(possible) || possible.length === 0) return fallbackItems;

  return possible.map((item: any, index: number) => {
    const email = item?.email || item?.client_email || item?.owner_email || item?.customer_email || "";
    const client = item?.client || item?.clientName || item?.client_name || item?.owner_name || item?.name || item?.full_name || item?.llc_name || item?.company_name || `Client ${index + 1}`;
    const llc = item?.llc || item?.llcName || item?.llc_name || item?.company || item?.company_name || item?.business_name || client;
...
export default function CleanAdminDashboard({ lang }: { lang: Lang }) {
  ...
  const payments = Math.max(items.filter((x) => String(x.payment || "").toLowerCase().includes("vérification")).length, 12);
      ...
      <div style={{ marginTop: 28, background: "#ffffff", borderRadius: 32, padding: 24, border: "1px solid #E5EAF2" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 16, marginBottom: 20 }}>
          <input
            ...
            style={{ height: 54, border: "1px solid #DDE5F0", borderRadius: 16, padding: "0 20px", fontWeight: 800, color: "#123A63", outline: "none" }}
          />
        ...
        <div style={{ border: "1px solid #DDE5F0", borderRadius: 18, overflow: "hidden" }}>
          ...
          {filtered.map((item, index) => {
            ...
            return (
              <div key={`${item.email || item.client || index}-${index}`} style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr 1fr 1fr 1.2fr 1.2fr 110px", padding: "18px 20px", borderTop: "1px solid #E5EAF2", alignItems: "center", color: "#123A63", fontWeight: 900, minHeight: 64 }}>
