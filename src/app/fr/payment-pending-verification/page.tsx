...
export default function PaymentPendingVerificationPage() {
  ...
  async function createClientSpace() {
    ...
    try {
      ...
      setMessage("Votre espace client a été créé. Redirection en cours...");

      setTimeout(() => {
        window.location.href = `/fr/espace-client?email=${encodeURIComponent(email)}`;
      }, 900);
    } catch (e: any) {
